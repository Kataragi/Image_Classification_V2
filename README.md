...
学習時の画像解像度を指定できます。解像度は固定で、学習中に変更されません。
...
# 512x512で学習
python train.py --dataset dataset --resolution 512

# 高解像度で学習 (1024x1024)
python train.py --dataset dataset --resolution 1024 --batch-size 8
```

**解像度の選び方:**
- **384**: 高速に実験、初期トレーニング、小規模GPU
- **512**: バランスが良い、ほとんどのケースで推奨
- **768**: 高精度が必要な場合
- **1024**: 最高精度、大容量GPUが必要

**注意:**
- 解像度が高いほどGPUメモリを多く消費します
- 解像度が高い場合は `--batch-size` を小さくしてください
- 学習中に解像度は変更されません（固定）
...
1. **シンプルな固定解像度学習に変更**
   - **削除**: 段階的解像度学習機能を完全削除
   - **削除**: Val lossスパイク検出とロールバック機能を削除
   - **削除**: `--resolution-threshold` オプションを削除
   - **追加**: `--resolution` オプション（384/512/768/1024から選択）
   ...
   **使用例:**
   ...
   # 1024x1024で学習
   python train.py --dataset dataset --resolution 1024
   ```

**破壊的変更:**
- `--resolution-threshold` オプションが削除されました
- 段階的解像度学習が行われなくなりました
- より単純で明確な動作になりました

**理由:**
- 複雑な自動制御よりも、ユーザーが解像度を明示的に指定する方がシンプル
- 学習の挙動が予測しやすくなる
- デバッグが容易になる
